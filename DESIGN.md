...
- 电量百分比：BAS 2A19 优先；无 BAS 时线性换算 `(mv-3200)/1000*100`，钳制 0-100。
  固件每个周期先推 BAS 再推 device_status，App 侧 device_status 只更新电压/温度/flags，
  **不得覆盖** BAS 百分比（否则线性兜底会在 LiPo 平台区把电量高估数倍）。
  连接建立时 BAS 可能是固件缓存值（开机/上次连接写入，连接后 5s 才刷新），
  读数需连续两次相差 ≤3 个百分点才置 `batteryValid=true`，期间 UI 显示 "—" 占位。
...
- 重连：每轮等待退避延迟后直连上次 MAC，等待 onReady（成功）或失败信号（本轮失败），15s 为单轮上限
  - 骑行中意外断线（自动重连）：最多 10 次，可能只是暂时超出范围，给足机会
  - 手动「重连上次设备」：最多 3 次，失败提示"未找到设备：请确认设备已开机并在附近"
  - 连接/重连/扫描中，Dashboard 底部操作栏提供「取消/停止」，随时终止重试
- 连接失败信号：库对"从未建立连接"的尝试**不会**回调 `onServicesInvalidated`，只通知
  `ConnectionObserver.onDeviceFailedToConnect`；此前只监听前者，失败尝试会把状态卡死在 Connecting
  （"重连偶发失效、只能手动取消"）。`Connecting` 收到该信号 → 进入重连循环；循环内本轮失败 →
  立即下一轮，不再盲等固定时长
- 所有连接回调带 manager 身份校验：上一轮被 `close()` 实例的迟到事件不影响新一轮连接
- 手动「重连上次设备」先按 MAC 短扫描（≤5s）预热协议栈设备记录：命中用 `ScanResult.device` 连接
  （自带地址类型），未命中或扫描不可用退回裸地址直连。未配对设备的地址类型只在蓝牙进程内存、不落盘，
  蓝牙进程重启/记录被淘汰后即失效——即"过几天连不上一次、扫描一次又好"的来源（自动重连不做扫描：
  后台扫描受限，且断线时记录仍是新的）
- 用户主动断开后不会自动重连
...
- 「灯带设置」页提供「悬浮窗指示」开关；偏好持久化且**默认开启**——连接就绪后由 Dashboard 自动启动，手动关闭后保持关闭
- 窗口创建晚于 BLE 数据时（首次进入、服务重启、重新进入骑行），`OverlayWindowHolder.attach` 回放缓存的连接/威胁度，OverlayService 再用仓库当前值校准；否则 StateFlow 不会为新窗口重发旧值，灯带会停在威胁度 0 直到下一次数值变化
...
- 电量卡：百分比 + 电压 + 进度条（绿/黄/红，与连接蓝解耦），常驻首屏；
  读数未稳定前百分比/电压显示 "—" 占位（灰色），稳定后再显示具体数值
